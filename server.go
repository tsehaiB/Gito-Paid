...
func callGeminiWithFiles(messages []openai.ChatCompletionMessage, files []struct {
	Name    string
	Content []byte
	Type    string
}) (string, error) {
	...
	for _, file := range files {
		mimeType := mime.TypeByExtension(file.Type)
		if mimeType == "" {
...
func processChat(userID, userInput string, files []struct {
	...
	if len(files) > 0 {
		...
		response, resErr = callGeminiWithFiles(messages, files)
		if resErr != nil {
			response = fmt.Sprintf("Error analyzing files: %v", resErr)
		}
	} else {
		// Original TogetherAI flow
		reqBody := struct {
			Model    string                         `json:"model"`
			Messages []openai.ChatCompletionMessage `json:"messages"`
		...
		if resErr := json.NewDecoder(resp.Body).Decode(&apiResult); resErr != nil {
...
func chatHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Extract userID and userMessage
	...
	// Process the chat (this now handles files automatically)
	response := processChat(userID, userMessage, files)
